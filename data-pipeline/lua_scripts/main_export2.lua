...
-- ============================================================
-- Mac：不切换工程导出（与 Windows 版一致）
-- 复制选定 Item 的 WAV → 生成 RPP → 外部渲染，不关闭原工程
-- ============================================================
...
function CollectSelectedItemsMedia()
    ...
    for i = 0, numItems - 1 do
        ...
        if item then
            local take = reaper.GetActiveTake(item)
            if not take then
                -- 空 Item（folder item / empty item）：无音频源，但仍需保留在胶囊中
                reaper.ShowConsoleMsg("  检测到空 Item（无 Take），保留在胶囊中\n")
                local track = reaper.GetMediaItemTrack(item)
                                                        ...
                                                        if fullPath then
                                                            ...
                                                            if f then
                                                                ...
                                                                if not mediaFiles[fullPath] then
                                                                    ...
                                                                    reaper.ShowConsoleMsg("    ✓ 子轨道媒体: " .. baseName .. "\n")
                                                                end
                                                            end
                                                        end
                                                    end
                        ...
                        if source then
                            ...
                            if fileName and fileName ~= "" and fileName ~= "?" then
                                ...
                                if isAbsolute then
                                    ...
                                    if not fullPath and (not currentProjDir or currentProjDir == "") then
                                        local baseName = string.match(fileName, "([^/\\]+)$") or fileName
                                        local tryDirs = { os.getenv("HOME") .. "/Music", os.getenv("HOME") .. "/Desktop", "/tmp" }
                                        for _, d in ipairs(tryDirs) do
                                            if d then
                                                local p = JoinPath(d, baseName)
                                                local f = io.open(p, "r")
                                                if f then f:close(); fullPath = p; break end
                                            end
                                        end
                                    end
                                end
...
function GenerateCapsuleRPP(outputDir, capsuleName, pathMapping, renderPreview, startTime, endTime, hasMidiItems)
    ...
    -- 收集需要保留的轨道（依赖追踪）
    -- RPP 中 <TRACK> 块顺序与 REAPER 轨道顺序一致，currentTrackNum 1 = 第 1 个 TRACK 块，对应 API 轨道号 1
    ...
    for i = 0, numItems - 1 do
        local item = reaper.GetSelectedMediaItem(0, i)
        if item then
            -- 收集 item 的 GUID（用于 RPP 中按 IGUID 精确匹配）
    ...
    -- 删除不相关的 TRACK 块，并建立旧轨道号 -> 新轨道号 映射
    ...
    for line in content:gmatch("([^\r\n]*)\r?\n?") do
        ...
        elseif inTrack then
            ...
            if line:match("^%s*<") and not line:match("^%s*<[^>]*>%s*$") then trackDepth = trackDepth + 1 end
    ...
    for origPath, _ in pairs(pathMapping) do
        ...
        if baseName then selectedMediaNames[baseName:lower()] = true end
    end
    ...
    for line in content:gmatch("([^\r\n]*)\r?\n?") do
        ...
        elseif inItem then
            ...
            if itemDepth == 0 then
                ...
                else removedCount = removedCount + 1 end
                inItem = false
                itemContent = ""
            end
        else
    ...
    for origPath, newPath in pairs(pathMapping) do
        ...
        local pathVariants = { origPath, sourceMediaFolder .. "/" .. baseName, "Audio/" .. baseName, baseName }
        if sourceMediaFolder ~= "Audio" then
            table.insert(pathVariants, 3, sourceMediaFolder .. "\\" .. baseName)
        end
        for _, variant in ipairs(pathVariants) do
            ...
            if count > 0 then content = replaced; replacedCount = replacedCount + count end
        end
    end
    ...
    if currentProjDir2 and currentProjDir2 ~= "" then
        content = content:gsub('(FILE%s+")([^"]-)"', function(prefix, filePath)
            ...
            if filePath:match("^/") or filePath:match("^[A-Za-z]:") or filePath:match("^Audio/") then
    ...
    if renderPreview then
        ...
        -- RENDER_RANGE 2 = 时间选区，后两参为 start/end，必须与 SELECTION 一致，否则渲染范围错误（MIDI/音频会错或无声）
...
RENDER_1X %d
        ...
        if content:match("SELECTION2%s+[%d%.%-]+%s+[%d%.%-]+") then content = content:gsub("SELECTION2%s+[%d%.%-]+%s+[%d%.%-]+", selection2Str) end
        if content:match("SELECTION%s+[%d%.%-]+%s+[%d%.%-]+") then content = content:gsub("SELECTION%s+[%d%.%-]+%s+[%d%.%-]+", selectionStr) end
        if content:match('RECORD_PATH "[^"]*"') then content = content:gsub('RECORD_PATH "[^"]*"', 'RECORD_PATH "Audio"') end
    end
    ...
    if not targetFile then
        ...
        if tempRppPath then os.remove(tempRppPath) end
        return nil
    end
...
-- 生成 metadata.json（不切换工程版）
local function generateUUIDForCapsule()
...
function GenerateCapsuleMetadata(outputDir, capsuleName, capsuleType, itemsInfo, mediaFiles, failedFiles)
    reaper.ShowConsoleMsg("\n=== 生成 metadata.json ===\n")
    local uuid = generateUUIDForCapsule()
    ...
    local json = string.format([[
{
  ...
  "preview_audio": "%s.ogg"
        ...
        uuid, capsuleName, capsuleType or "magic", os.date("%Y-%m-%dT%H:%M:%S"),
        bpm, duration, sampleRate, #itemsInfo, mediaCount, failedCount,
        pluginListJson, #plugins,
        tostring(routingInfo.has_sends), tostring(routingInfo.has_folder_bus), routingInfo.tracks_included,
...
function SaveProjectWithMedia(targetPath)
    ...
    -- 媒体文件：优先从当前工程中的 Item（API）收集，只复制当前时间线上实际使用的媒体；
    -- RPP 中可能包含整个媒体池（含未用源），会导致复制全部 WAV。
    local originalProjDir = string.match(currentProjPath, "(.+)/") or ""
    local mediaFiles = {}
    local trackCount = reaper.CountTracks(0)
    local currentProjDir = string.match(currentProjPath, "(.+)/") or ""

    reaper.ShowConsoleMsg("收集媒体文件（仅当前工程中的 Item 使用）...\n")
                    ...
                    if take ~= nil then
                        ...
                        if source ~= nil then
                            ...
                            if not retval or fileName == "" or fileName == "?" then
                                ...
                                if takeSource then fileName = tostring(takeSource) end
                            end
                            if not fileName or fileName == "" or fileName == "?" then
                                local _, itemSource = reaper.GetSetMediaItemInfo_String(item, "P_SOURCE", "", false)
                                if itemSource and itemSource ~= "" then fileName = itemSource end
                            end
                            if fileName and fileName ~= "" and fileName ~= "?" then
                                local fullPath = nil
                                if string.match(fileName, "^/") then
                                    ...
                                    for _, testPath in ipairs(testPaths) do
                                        ...
                                        if file then file:close(); fullPath = testPath; break end
                                    end
                                end
                                if fullPath then
    ...
    if mediaCount == 0 then
        reaper.ShowConsoleMsg("当前工程中无媒体 Item，尝试从 RPP 解析...\n")
        mediaFiles = ExtractMediaFilesFromRPP(targetPath, originalProjDir, sourceMediaFolder)
        mediaCount = (function() local count = 0; for _ in pairs(mediaFiles) do count = count + 1 end; return count end)()
    end
    reaper.ShowConsoleMsg("共 " .. mediaCount .. " 个媒体文件需要复制\n")
...
function ExportCapsule()
    ...
    if itemCount == 0 then
        ...
        return false, "没有选中的 Audio Item，请在 REAPER 中选中要导出的 Item"
    end
    ...
    -- 不切换工程流程：复制选定 Item 的 WAV → 生成 RPP → 外部渲染（与 Windows 一致）
    ...
    if mediaCount == 0 and not hasItems then
        if not _SYNEST_AUTO_EXPORT then
            reaper.ShowMessageBox("没有找到媒体文件", "错误", 0)
        else
            reaper.ShowConsoleMsg("[自动导出] 错误: 没有找到媒体文件\n")
        end
        return false, "没有找到媒体文件（选中的 Item 可能无有效媒体源）"
    end
    if mediaCount == 0 and hasItems then
